          ...
          w.JS_Basic.Ajax(
            ...
            function(result) {
              ...
              if (resultData.code == 0) {
                // 手机存储
                let config = w.module.config();
                config.w({ w: resultData.w }, (ret, err) => {
                  w.function.msg(JSON.stringify(ret), 1);
                  if (ret.w == "success") {
                    window.localStorage.setItem("w", resultData.w);
                    location.href = api.wgtRootDir + this_.w.url.main;
                  }
                  else { w.function.msg("数据保存错误", 2);}
                });
      ...
      mounted() {
        ...
        this.w.function.isLogin(
          () => {
            ...
            // api.openWin({
            //   name: "main",
            //   url: api.wgtRootDir + this.w.url.main
            // });
            location.href = api.wgtRootDir + this.w.url.main;
          },
          () => {
            //登录失败
            this.ready_();
